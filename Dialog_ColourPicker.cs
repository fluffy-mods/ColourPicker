...
namespace ColourPicker
{
    public class Dialog_ColourPicker : Window
    {
        ...
        private void CreateAlphaBG(ref Texture2D bg, int width, int height)
        {
            ...
            var row = 0;
            for (var x = 0; x < width; x = x + _alphaBGBlockSize)
            {
                var column = row;
                for (var y = 0; y < height; y = y + _alphaBGBlockSize)
                {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            var hueHandleRect = new Rect(hueRect.xMin - 3f, hueRect.yMin + _huePosition - _handleSize / 2,
                                         _sliderWidth + 6f, _handleSize);
            var alphaHandleRect = new Rect(alphaRect.xMin - 3f, alphaRect.yMin + _alphaPosition - _handleSize / 2,
                                           _sliderWidth   + 6f, _handleSize);
            var pickerHandleRect = new Rect(pickerRect.xMin + _position.x - _handleSize / 2,
